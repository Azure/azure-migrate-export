...
namespace Azure.Migrate.Export.Assessment.Parser
{
    public class AzureSQLAssessedInstancesParser
    {
            ...
            foreach (var kvp in AzureSQLAssessmentStatusMap)
            {
                ...
                while (!string.IsNullOrEmpty(url))
                {
                    ...
                    foreach (var value in assessedInstancesObj.Values)
                    {
                        ...
                        UpdateAssessedInstancesDataset(AzureSQLInstancesData, key, value, kvp.Key, userInputObj);
        ...
        private void UpdateAssessedInstancesDataset(Dictionary<string, AzureSQLInstanceDataset> AzureSQLInstancesData, string key, AzureSQLAssessedInstanceValue value, AssessmentInformation assessmentInfo, UserInput userInputObj)
        {
            ...
            AzureSQLInstancesData[key].MonthlyAzureSiteRecoveryCostEstimate = UtilityFunctions.GetAzureSiteRecoveryMonthlyCostEstimate(userInputObj);
            AzureSQLInstancesData[key].MonthlyAzureBackupCostEstimate = UtilityFunctions.GetAzureBackupMonthlyCostEstimate(AzureSQLInstancesData[key].LogicalDisks, userInputObj);
        }
