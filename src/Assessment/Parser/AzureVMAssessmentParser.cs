...
namespace Azure.Migrate.Export.Assessment.Parser
{
    public class AzureVMAssessmentParser
    {
            ...
            foreach (var kvp in AzureVMAssessmentStatusMap)
            {
                ...
                while (!string.IsNullOrEmpty(url))
                {
                    ...
                    foreach (var value in obj.Values)
                    {
                        ...
                        UpdatePerformanceBasedDataset(AzureVMPerformanceBasedMachinesData, key, value, kvp.Key, userInputObj.ExchangeRateUSD);
        ...
        private void UpdatePerformanceBasedDataset(Dictionary<string, AzureVMPerformanceBasedDataset> AzureVMPerformanceBasedMachinesData, string key, AzureVMAssessedMachineValue value, AssessmentInformation assessmentInfo, double exchangeRate)
        {
            ...
            AzureVMPerformanceBasedMachinesData[key].AzureSiteRecoveryMonthlyCostEstimate = UtilityFunctions.GetAzureSiteRecoveryMonthlyCostEstimate(exchangeRate);
            AzureVMPerformanceBasedMachinesData[key].AzureBackupMonthlyCostEstimate = UtilityFunctions.GetAzureBackupMonthlyCostEstimate(AzureVMPerformanceBasedMachinesData[key].Disks, exchangeRate);
